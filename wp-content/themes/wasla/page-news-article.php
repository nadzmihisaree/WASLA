<?php

/**
 * Template Name: News Article
 * Created on : March 25, 2022
 * @author    : Nanduni Mihisaree
 */
get_header(); ?>

<div class=" row news-article mx-auto" style=" max-width: 892px;padding-bottom: clamp(50px,14vw,212px); padding-top: clamp(20px,4vw,68px);">

    <!-- Section - 01 (head) -->
    <p class="fs-35 fw-600 mb-4" style="color: #484848;">Mauris facilisis libero 50 hendrerit ipsum primis in faucibus..</p>
    <p class="fs-20 fw-500" style=" color: rgba(139, 139, 139, 0.95); margin-bottom: clamp(20px,4vw,68px);">22 May 2021</p>

    <img src="<?php bloginfo('template_directory'); ?>/assets/images/group522.png" class="news-inner-img mb-5" alt="">
    <!--/ Section - 01 (head) -->

    <!-- Section - 02 (content) -->
    <p class="fs-18" style="color: #3c3c3c;text-align: justify;">111Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aenean et porttitor mauris. Quisque convallis urna ac lectus pulvinar vulputate. Interdum et malesuada fames ac ante ipsum primis in faucibus. Duis luctus arcu in risus aliquet, vel tempor justo convallis. Donec ac arcu et arcu rhoncus vulputate sed pellentesque justo.
        <br><br>
        Vivamus urna odio, luctus nec lacus eget, blandit suscipit leo. Aliquam molestie metus magna, at euismod sapien eleifend volutpat. Aliquam interdum et dui in pretium. Proin quis purus sem. Cras egestas suscipit massa vel ultricies. Orci varius natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Sed aliquet aliquam sem, eget dapibus diam fermentum sed. Vivamus congue odio non nisl congue pulvinar. Proin viverra urna eu vulputate volutpat. Quisque sed nisi erat. Aenean at urna lacus. Pellentesque sagittis, lacus ut fermentum faucibus, sapien nisl consequat nibh, vel fermentum tellus ante sit amet quam.
        <br><br>
        Ut ultricies enim quis dui molestie mollis. Aliquam sed viverra est. Phasellus tellus nulla, eleifend non pellentesque at, consequat et nisl. Suspendisse lacinia rhoncus tortor, at interdum massa posuere placerat. Suspendisse lacus tellus, convallis ut tortor ac, bibendum ullamcorper tortor. Morbi ac quam non ligula varius euismod. Sed in hendrerit odio. Maecenas tincidunt quis lacus ut fringilla. Quisque quis mi egestas, tincidunt mi eget.
        <br><br>
        Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aenean et porttitor mauris. Quisque convallis urna ac lectus pulvinar vulputate. Interdum et malesuada fames ac ante ipsum primis in faucibus. Duis luctus arcu in risus aliquet, vel tempor justo convallis. Donec ac arcu et arcu rhoncus vulputate sed pellentesque justo.
    </p>
    
    <!--/ Section - 02 (content) -->
</div>


<?php get_footer() ?>